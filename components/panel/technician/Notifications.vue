  ...
  <div class="container">
    ...
    <div class="faq">
        ...
        >
          <b-card no-body>
            <b-card-header header-tag="header" role="tab">
              <a @click.prevent="read(notification.id)" href>
                <b-button
                  block
      ...
      <div class="pagination">
        <div v-for="page in pagination" :key="page.label">
          <a
            v-if="page.label == '&laquo; Previous'"
            @click.prevent="paginate(page.url)"
          >
            <span class="next-previous" id="previous"></span>
          </a>
          <a
            :class="{ active: page.active }"
            v-if="
              page.label != 'Next &raquo;' && page.label != '&laquo; Previous'
            "
            @click.prevent="paginate(page.url)"
            >{{ page.label }}</a
          >
          <a
            v-if="page.label == 'Next &raquo;'"
            @click.prevent="paginate(page.url)"
          >
            <span class="next-previous" id="next"></span>
          </a>
        </div>
      </div>
    </div>
    ...
    fetchNotifications() {
      Axios.get(this.$hostname + "/notifications", {
        ...
        .then((resp) => {
          ...
          this.pagination = resp.data.notifications.links;
          this.loading = false;
        })
        .catch((err) => {
          this.$router.push("/");
...
.card {
  border-bottom: solid 2px #7ab3837e;
  padding-bottom: 10px;
  padding-top: 10px;
}
.title {
...
.title .right-side .unread {
  background-image: url("../../../assets/Caution_green.svg");
  background-repeat: no-repeat;
  background-size: contain;
  background-position: center;
  height: 20px;
...
.left-side .arrow {
  background-image: url("../../../assets/panel/green/down.svg");
  background-repeat: no-repeat;
  background-size: contain;
  background-position: center;
  height: 15px;
...
span#next {
  background-image: url("../../../assets/panel/green/right.png");
}
span#previous {
  background-image: url("../../../assets/panel/green/left.png");
}
