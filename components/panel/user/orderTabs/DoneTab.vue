<template>
  <div>
    <div v-if="!loading">
      <div class="order" v-for="order in done_orders.reverse()" :key="order.id">
        <a :href="'/order/done/' + order.id">
          <div class="row-one">
...
export default {
  ...
  methods: {
    fetchOrders() {
      Axios.get(this.$hostname + "/done-orders", {
        headers: {
        ...
        .then((resp) => {
          this.loading = false;
          this.done_orders = resp.data.orders;
        })
        .catch((err) => {
          console.log(err, "can not fetch done orders");
        });
    },
  },
  data() {
