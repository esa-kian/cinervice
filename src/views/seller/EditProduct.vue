...
export default {
  ...
  methods: {
    fetch() {
      this.$http
        .get(this.$hostname + "/product?id=" + this.id)
        .then((resp) => {
          console.log(resp.data.item);
          this.name = resp.data.item.name;
          this.description = resp.data.item.description;
          this.number = resp.data.item.number;
          this.price = resp.data.item.price;
          this.price_with_discount = resp.data.item.priceWithDiscount;
        })
        .catch((err) => {
    ...
    submit() {
      ...
      this.$http
        .put(this.$hostname + "/seller/product?id="+this.id, fd, this.$config)
        .then((resp) => {
